    ...
    def self.registered(app)
      app.get('/api/security/permissions') do
        ...
        JSON.fast_generate([
          ...
          {
            'name' => 'tech-leads',
            'uri'  => server_url.join('/api/security/permissions/Any Remote')
          }
        ])
      end

      app.get('/api/security/permissions/Any Remote') do
        content_type 'application/vnd.org.jfrog.artifactory.security.PermissionTargets+json'
        JSON.fast_generate(
          'name'             => 'Any Remote',
          'include_pattern'  => nil,
      ...
      app.put('/api/security/permissions/:name') do
        return 415 unless request.content_type == 'application/vnd.org.jfrog.artifactory.security.PermissionTargets+json'
